...
{
    sub_point_cloud_ = nh.subscribe("/top/rslidar_points", 10, &PclTestCore::point_cb, this);
...
void PclTestCore::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
	...
	tf_mat<<   0.999706 ,-0.00376436  ,  0.132762  ,  0.348397,
	0.0038821   , 0.999988 , -0.0265958 , -0.0741032,
				 -0.132759  , 0.0266127    ,0.999694  ,   0,
	0        ,   0     ,      0      ,     1;
//	tf_mat.setIdentity();
	pcl::transformPointCloud(*current_pc_ptr,*temp,tf_mat);
	current_pc_ptr = temp;
//1. 去掉高的点
